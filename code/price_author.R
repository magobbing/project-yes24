...
sample_df <- readRDS("C:/Users/751mb/OneDrive - 전북대학교/문서/yes24/temp_books_with_price_partial.rds")

unprocessed_df <- books_df %>%
  filter(!(book_uid %in% sample_df$book_uid))
...
unprocessed_df2 <- readRDS("C:/Users/751mb/OneDrive - 전북대학교/문서/yes24/temp_unprocessed_df_partial.rds")


# 행 단위로 병합
combined_df <- bind_rows(unprocessed_df, sample_df)
